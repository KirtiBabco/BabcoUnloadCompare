...
<section class="card search-card compact-card"><div class="field grow"><label>PO Number</label><div class="search-line"><input id="poSearch" class="input input-lg" placeholder="Enter PO number" autocomplete="off"/><button type="button" id="searchPoBtn" class="btn primary" onclick="UC.searchPO()">Search PO</button></div><div id="poResults" class="search-results hidden"></div></div><div class="workflow"><span class="active">1 PO</span><i>→</i><span>2 Count</span><i>→</i><span>3 Verify</span><i>→</i><span>4 Complete</span></div></section>
<section id="receivingSection" class="hidden">
  ...
  <section class="toolbar card compact-toolbar"><div class="toolbar-group"><button type="button" class="btn excel-download" onclick="UC.exportExcel()">Download Excel</button><button type="button" class="btn" onclick="UC.uploadExcel()">Upload Filled Excel</button><input type="file" id="excelFile" accept=".xls,.xlsx" hidden/><button type="button" class="btn" onclick="UC.uploadDocument()">Receiving Sheet</button><input type="file" id="docFile" accept=".pdf,.jpg,.jpeg,.png" hidden/></div><div class="toolbar-group"><button type="button" class="btn" onclick="UC.save('In Progress')">Save</button><button type="button" class="btn success" onclick="UC.save('Ready for Verification')">Ready to Verify</button><button type="button" class="btn" onclick="UC.printReport()">Print</button></div></section>
...
<div id="importModal" class="modal hidden"><div class="modal-box wide"><div class="modal-head"><div><span class="eyebrow">FILLED EXCEL IMPORT PREVIEW</span><h3>Review before applying</h3></div><button type="button" class="icon-btn" onclick="UC.closeImport()">×</button></div><div id="importSummary"></div><div class="table-wrap"><table><thead><tr><th>Row</th><th>SKU</th><th>Item</th><th>Qty</th><th>Result</th></tr></thead><tbody id="importRows"></tbody></table></div><div class="modal-actions"><span class="muted">Unmatched rows are never silently discarded.</span><button type="button" class="btn primary" id="applyImportBtn" onclick="UC.applyImport()">Apply matched rows</button></div></div></div>
<div id="toast" class="toast hidden"></div>
</asp:Content>
<asp:Content ID="s" ContentPlaceHolderID="ScriptContent" runat="server"><script src="Scripts/unload-compare.js"></script></asp:Content>
